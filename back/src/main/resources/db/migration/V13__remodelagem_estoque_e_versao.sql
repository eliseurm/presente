...
DROP TABLE IF EXISTS presente_sh.produto_cor;
DROP TABLE IF EXISTS presente_sh.produto_tamanho;

-- 2. Criar a nova tabela ProdutoEstoque
-- Nota: Assumi o nome da tabela como 'produto_estoque' para não conflitar com 'produto'
CREATE TABLE IF NOT EXISTS presente_sh.produto_estoque (
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.constraint_column_usage WHERE constraint_name = 'fk_estoque_produto') THEN
       ALTER TABLE presente_sh.produto_estoque ADD CONSTRAINT fk_estoque_produto FOREIGN KEY (produto_id) REFERENCES produto (id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.constraint_column_usage WHERE constraint_name = 'fk_estoque_cor') THEN
       ALTER TABLE presente_sh.produto_estoque ADD CONSTRAINT fk_estoque_cor FOREIGN KEY (cor_id) REFERENCES cor (id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.constraint_column_usage WHERE constraint_name = 'fk_estoque_tamanho') THEN
       ALTER TABLE presente_sh.produto_estoque ADD CONSTRAINT fk_estoque_tamanho FOREIGN KEY (tamanho_id) REFERENCES tamanho (id);
    END IF;
END $$;
