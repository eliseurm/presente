...
if [ "${USE_CONNECTOR}" = "true" ]; then
  ...
  # Segue o padrão do projeto de referência (extrato): usar SPRING_DATASOURCE_URL
  # com Cloud SQL Connector, host "google" e SocketFactory do Connector.
  #  - ipTypes=PUBLIC usa IP público (sem exigir Private IP/VPC)
  #  - currentSchema mantém compatibilidade
  #  - enableIamAuth=false evita tentativa de IAM auth via IAM
  ENV_VARS_STRING="${ENV_VARS_STRING},SPRING_DATASOURCE_URL=jdbc:postgresql://google/${DB_NAME}?cloudSqlInstance=${INSTANCE_CONNECTION_NAME}&socketFactory=com.google.cloud.sql.postgres.SocketFactory&ipTypes=PUBLIC&currentSchema=presente_sh&enableIamAuth=false"
else
  : "${DB_HOST:?Defina DB_HOST quando USE_CONNECTOR=false}"
  ENV_VARS_STRING="${ENV_VARS_STRING},SPRING_DATASOURCE_URL=jdbc:postgresql://${DB_HOST}:${DB_PORT}/${DB_NAME}?currentSchema=presente_sh"
fi
